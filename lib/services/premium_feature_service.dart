...
class PremiumFeatureService {
  ...
  Future<bool> canAccessFeature(String producerId, String featureId) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('${constants.getBaseUrl()}/api/subscription/check-feature-access'),
        headers: {'Content-Type': 'application/json'},
        // Envoyer les paramètres dans le query string
        // L'API attend producerId et featureId pour vérifier l'accès
        // Le backend utilisera le middleware requireFeature pour vérifier les droits
      );
